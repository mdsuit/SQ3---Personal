package ClassesObjects;

/**
 * Created by MDS1728 on 5/9/2017.
 */
public class ShirtArray {
    public static void main(String[] args) {
        Shirt nathan = new Shirt();
        Shirt michael = new Shirt();
        Shirt reggie = new Shirt();

        //Create array of designers objects
        Shirt[] designers = {new Shirt('B',21.99,"A12345"),
                new Shirt('G',19.99,"A34567"),
                new Shirt('W',25.99,"A98765")};


        //need to assign Nahan's Info
        /*
        nathan.colorCode = 'B';
        nathan.itemId = "A12345";
        nathan.price = 21.99;


        designers[0].colorCode = 'B';
        designers[0].itemId = "A12345";
        designers[0].price = 21.99;


        michael.colorCode = 'G';
        michael.itemId = "A34567";
        michael.price = 19.99;


        designers[1].colorCode = 'G';
        designers[1].itemId = "A34567";
        designers[1].price = 19.99;


        reggie.colorCode = 'W';
        reggie.itemId = "A98765";
        reggie.price = 25.99;


        designers[2].colorCode = 'B';
        designers[2].itemId = "A98765";
        designers[2].price = 25.99;

        //Verify all resources designs
        /
        nathan.printFields();
        System.out.println("----------------------------");
        michael.printFields();
        System.out.println("----------------------------");
        reggie.printFields();



        designers[0].printFields();
        System.out.println("-----------------");
        designers[1].printFields();
        System.out.println("-----------------");
        designers[2].printFields();


        for (Shirt temp:designers){
            temp.printFields();
            System.out.println("------------------");
            */
        }
    }


